...
renderer_init(RnState* state) {
  ...
  "in vec2 v_max_coord;\n"
  "\n"
  "  const float shadow_softness = 0.0f;\n"
  "  const vec2 shadow_offset = vec2(0.0f, 10.0f);\n"
  ...
  "\n"
  "if (u_screen_size.y - gl_FragCoord.y  < v_min_coord.y - shadow_offset.y + shadow_softness - bias && v_min_coord.y != -1) {\n"
  "    discard;\n"
  "}\n"
  "if (u_screen_size.y - gl_FragCoord.y > v_max_coord.y  + shadow_offset.y + shadow_softness + bias && v_max_coord.y != -1) {\n"
  "    discard;\n"
  "}\n"
  "if ((gl_FragCoord.x < v_min_coord.x + shadow_offset.x - shadow_softness - bias && v_min_coord.x != -1) || \n"
  "    (gl_FragCoord.x > v_max_coord.x + shadow_offset.x + shadow_softness + bias && v_max_coord.x != -1)) {\n"
  "    discard;\n"
  "}\n"
  " if(v_is_text == 1.0) {\n"
  ...
  "   if (frag_pos.x < pos_adjusted.x - shadow_offset.x - shadow_softness || frag_pos.x > bottom_right.x + shadow_offset.x + shadow_softness ||\n"
  "     frag_pos.y < pos_adjusted.y - shadow_offset.y - shadow_softness || frag_pos.y > bottom_right.y + shadow_offset.x + shadow_softness) {\n"
  "       discard;\n"
  "   }\n"
  "   }\n"
...
"                                               (1.0f - smoothstep(v_border_width - border_softness, v_border_width, abs(distance)));\n"
  ...
  "    half_size, corner_radius\n"
  "  );\n"
  "  float shadow_alpha = 1.0f - smoothstep(-shadow_softness, shadow_softness, shadow_distance);\n"
  "  vec4 shadow_color = vec4(0.4f, 0.4f, 0.4f, 1.0f);\n"
  "  vec4 final_color = mix(res_color, shadow_color, shadow_alpha - smoothed_alpha);\n"
  "  o_color = final_color;\n"
  "}\n"
  "\n"
  ""
